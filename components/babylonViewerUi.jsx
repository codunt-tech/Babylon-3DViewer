    ...
    switch (icon) {
        case '📦': return (
            <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
                <rect x="3" y="7" width="18" height="13" rx="2"/><path d="M16 3v4M8 3v4M3 11h18"/>
            </svg>)
        case '👁️':
        case '👁️‍🗨️': return (
            <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
        ...
        case '🚢': return (
            <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
                <path d="M2 20a2.4 2.4 0 0 0 2 1 2.4 2.4 0 0 0 2-1 2.4 2.4 0 0 1 2-1 2.4 2.4 0 0 1 2 1 2.4 2.4 0 0 0 2 1 2.4 2.4 0 0 0 2-1 2.4 2.4 0 0 1 2-1 2.4 2.4 0 0 1 2 1"/><path d="M4 18V9l8-4 8 4v9"/><path d="M8 13h8"/>
            </svg>)
        case '🔍': return (
            <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
...
const ContextMenu = React.memo(({ position, visible, selectedCompartment, selectedPart, viewMode, onClose, onAction }) => {
    if (!visible) return null

    const isPart = !!(selectedPart && typeof selectedPart === 'string')
    ...
    if (viewMode === 'asset') {
        ...
        subtitle = isPart ? selectedPart.split('-').slice(1).join('-') : selectedCompartment?.replace(/_/g, ' ') || ''
        menuItems = [
        ...
        title    = 'Hullpart view'
        subtitle = isPart ? selectedPart.split('-').slice(1).join('-') : ''
        menuItems = [
            { label: 'Asset view',       action: 'backToAsset',       icon: '🚢' },
    ...
    return (
        ...
        >
            ...
            <div style={{ padding: '12px 16px 10px', borderBottom: '1px solid #f0f0f0' }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: '#2196F3', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 2 }}>
                    {title}
                </div>
...
// FIX: icon SVG components extracted to avoid re-creating JSX in render loops
const IconAnchor = () => (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#77b3ff" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
        <path d="M12 3v6"/><circle cx="12" cy="4" r="1.5"/><path d="M6 12a6 6 0 0 0 12 0"/><path d="M6 12H3m18 0h-3"/>
    </svg>)
const IconDrop = () => (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#77b3ff" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
...
const IconCube = () => (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#77b3ff" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
        <path d="M12 3l8 4.5v9L12 21 4 16.5v-9L12 3z"/><path d="M12 21v-9"/><path d="M20 7.5l-8 4.5L4 7.5"/>
    </svg>)
const IconDot = () => (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#77b3ff" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
...
// ─── HierarchicalSidebar ──────────────────────────────────────────────────────
// FIX: wrapped in React.memo with stable prop references to prevent unnecessary
// re-renders. The component was re-rendering on every mouse move / camera update.
...
const HierarchicalSidebar = React.memo(({
    ...
    // FIX: stable callbacks inside sidebar to avoid child re-renders
    const toggleFunc = useCallback((name) => {
        setExpandedFuncs((prev) => { const n = new Set(prev); n.has(name) ? n.delete(name) : n.add(name); return n })
    }, [])
    const toggleComp = useCallback((name) => {
        setExpandedComps((prev) => { const n = new Set(prev); n.has(name) ? n.delete(name) : n.add(name); return n })
    }, [])

    // Auto-expand the group containing the selected compartment
    useEffect(() => {
    ...
    const grouped = useMemo(() => {
        ...
        ;['plates', 'shells', 'brackets', 'stiffeners'].forEach((type) => {
            ;(shipData?.[type] || []).forEach((item) => { if (item?.compartmentName) all.add(item.compartmentName) })
        })
        const byFunc = {}
        ...
        return Object.keys(byFunc).sort().map((name) => ({ name, compartments: byFunc[name].sort() }))
    }, [shipData])

    // FIX: memoize hull parts so the list doesn't recompute on every render
    const hullPartsByCompartment = useMemo(() => {
        const map = {}
        Object.entries(loadedCompartments || {}).forEach(([cName, cData]) => {
            const parts = new Set()
            Object.values(cData?.loadedComponents || {}).forEach((comp) => {
                ;(comp?.meshes || []).forEach((mesh) => { if (mesh?.name) parts.add(mesh.name) })
            })
            map[cName] = Array.from(parts).sort((a, b) => a.localeCompare(b))
        })
        return map
    }, [loadedCompartments])
    ...
    return (
        <div
            style={{
                position: 'fixed', top: `${topOffset}px`, left: 0,
                width: '390px', height: `calc(100vh - ${topOffset}px)`,
                background: '#001b37', borderRight: '1px solid rgba(255,255,255,0.08)',
                color: 'rgba(255,255,255,0.92)', display: 'flex', flexDirection: 'column',
                zIndex: 1000, fontFamily: 'system-ui, -apple-system, Segoe UI, Roboto, sans-serif',
            }}
        >
            {/* Header */}
            <div style={{ padding: '14px 14px 10px', borderBottom: '1px solid rgba(255,255,255,0.08)', flexShrink: 0 }}>
                ...
                <div
                    style={{
                        ...
                        borderRadius: 8, border: '1px solid rgba(67,142,220,0.55)',
                        background: '#00254c', padding: '8px 12px',
                    }}
                >
                    ...
                    return (
                        <div key={group.name}>
                            ...
                            >
                                ...
                                <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: 0.3, flex: 1, textTransform: 'uppercase' }}>
                                    {group.name}
                                </span>
                                <span style={{ color: '#6ea8ea', fontSize: 14, flexShrink: 0 }}>
                                    {funcExpanded ? '▾' : '›'}
                                </span>
                            </div>
                            ...
                            {funcExpanded && (
                                <div style={{ paddingLeft: 8, marginTop: 4, marginBottom: 4 }}>
                                    <div style={{ fontSize: 10, letterSpacing: 0.5, color: 'rgba(255,255,255,0.45)', marginBottom: 4, paddingLeft: 4 }}>
                                        COMPARTMENTS
                                    </div>
                                            ...
                                            <div key={cName} style={{ marginBottom: 4 }}>
                                                ...
                                                {compExpanded && (
                                                    <div style={{ marginTop: 4, marginLeft: 14 }}>
                                                        <div style={{ fontSize: 10, letterSpacing: 0.5, color: 'rgba(255,255,255,0.4)', marginBottom: 3 }}>
                                                            HULLPARTS
                                                        </div>
                                                        {hullParts.length === 0 && !(loadedCompartments?.[cName]) && (
                                                            <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.45)', padding: '3px 6px' }}>
                                                                Click compartment to load…
                                                            </div>
                                                        )}
                                                        {hullParts.length === 0 && loadedCompartments?.[cName] && (
                                                            <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.45)', padding: '3px 6px' }}>
                                                        ...
                                                        {hullParts.map((meshName) => {
                                                            const partId     = `${cName}-${meshName}`
                                                            const hidden     = hiddenParts?.has(partId)
                                                            const partSelect = selectedPart === partId
                                                            return (
                                                                <div
                                                                    key={partId}
